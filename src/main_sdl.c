...
static int sister_presets_file_path(char *path, size_t path_size)
{
    char directory[TS_RUNTIME_PATH_MAX];
    if (runtime_path_parent(config_file_path(), directory, sizeof(directory)) &&
        runtime_join_path(path, path_size, directory, "sister-presets.ini"))
        return 1;
    return snprintf(path, path_size, "sister-presets.ini") >= 0 &&
           strlen("sister-presets.ini") < path_size;
}

static int fallout_presets_file_path(char *path, size_t path_size)
{
    char directory[TS_RUNTIME_PATH_MAX];
    if (runtime_path_parent(config_file_path(), directory, sizeof(directory)) &&
        runtime_join_path(path, path_size, directory, "fallout-presets.ini"))
        return 1;
    return snprintf(path, path_size, "fallout-presets.ini") >= 0 &&
           strlen("fallout-presets.ini") < path_size;
}
...
#undef ARP_HASH
    ...
    if (sister != NULL) {
        ...
        state_hash_bytes(&hash, sister->page_source_masks,
                         count < TS_SISTER_RUNTIME_PAGE_LIMIT ?
                         count * sizeof(sister->page_source_masks[0]) :
                           ...
                           const char *path)
{
    ...
    if (recipe && !ts_sister_project_state_load(
            &sister_state, path,
            audio->output_rate > 0 ? (uint32_t)audio->output_rate : 48000u,
            &sister_state_present, error, sizeof(error))) {
        snprintf(ui->status, sizeof(ui->status),
                 "LOAD FAILED: %.135s", error);
        return 0;
    }
    ...
    if (recipe) {
        ...
        if (ok && sister_state_present &&
            (sister_state.page_count != ts_sample_pages_count(sample_pages) ||
             sister_state.active_page != ts_sample_pages_active(sample_pages))) {
            snprintf(error, sizeof(error),
                     "Sister project state does not match the project page manifest");
            ok = 0;
        }
        if (ok) {
            ...
            (void)ts_sister_runtime_set_page(
                &audio->sister, ts_sample_pages_active(sample_pages), instrument);
            if (sister_state_present) {
...
typedef struct {
    ...
    TsRawImportSettings raw_settings;
    float waveform_minimum[2][TS_IMPORT_PREVIEW_COLUMNS];
    float waveform_maximum[2][TS_IMPORT_PREVIEW_COLUMNS];
    char path[TS_BROWSER_PATH_MAX];
    char error[160];
    int automatic;
    int ok;
...
static int import_cancel_check(void *userdata)
{
    ImportWorker *worker = (ImportWorker *)userdata;
    return worker != NULL && SDL_AtomicGet(&worker->cancel) != 0;
}
    ...
    for (int x = 0; x < TS_IMPORT_PREVIEW_COLUMNS; ++x) {
        ...
        for (uint8_t channel = 0u; channel < sample->channels; ++channel) {
            ...
            for (size_t frame = first; frame < last; ++frame) {
                float value = sample->data[frame * sample->channels + channel];
                if (value < low) low = value;
                if (value > high) high = value;
            }
            minimum[channel][x] = low;
            maximum[channel][x] = high;
        }
        if (sample->channels == 1u) {
            minimum[1][x] = minimum[0][x];
            maximum[1][x] = maximum[0][x];
        }
        if (cancel_check != NULL && cancel_check(cancel_userdata)) return 0;
    }
    return 1;
}

static int import_worker_main(void *userdata)
{
    ImportWorker *worker = (ImportWorker *)userdata;
    ...
    if (worker->automatic)
        ...
        worker->ok = ts_audio_import_decode_raw_cancelable(
            &worker->decoded, worker->path, &worker->raw_settings,
            import_cancel_check, worker,
            worker->error, sizeof(worker->error));
    if (worker->ok && !build_import_waveform_cache(
        ...
        worker->ok = 0;
        snprintf(worker->error, sizeof(worker->error), "Import cancelled");
    }
    SDL_AtomicSet(&worker->done, 1);
    return worker->ok ? 0 : 1;
}

static void discard_import_worker(ImportController *controller)
{
    ImportWorker *worker;
    if (controller == NULL || controller->worker == NULL) return;
    worker = controller->worker;
    SDL_AtomicSet(&worker->cancel, 1);
    SDL_WaitThread(worker->thread, NULL);
    ts_audio_import_free(&worker->decoded);
    free(worker);
    controller->worker = NULL;
                                 ...
                                 TsUiState *ui, ImportController *controller)
{
    ...
    discard_import_worker(controller);
    ...
    ui->import_preview_selection_anchor = 0u;
                                      ...
                                      const ImportController *controller)
{
    ...
    if (sample->frames > 0u && ui->import_preview_playhead >= sample->frames)
        ui->import_preview_playhead = sample->frames - 1u;
    if (ui->import_preview_selection_last > sample->frames)
        ui->import_preview_has_selection = 0;
...
static size_t import_preview_snap(const TsSample *sample, size_t requested)
{
    size_t radius;
    size_t first;
    size_t last;
    if (sample == NULL || sample->data == NULL || sample->frames == 0u) return 0u;
    if (requested >= sample->frames) requested = sample->frames - 1u;
    radius = sample->sample_rate > 0u ? sample->sample_rate / 20u : 2205u;
    if (radius < 32u) radius = 32u;
    first = requested > radius ? requested - radius : 0u;
    last = requested <= SIZE_MAX - radius ? requested + radius : SIZE_MAX;
                                    ...
                                    int x)
{
    ...
    ui->import_preview_selection_first = first;
                                ...
                                int bypass_preview)
{
    ...
    controller->bypass_preview = bypass_preview != 0;
    ts_raw_import_settings_default(&ui->import_raw_settings);
    worker = (ImportWorker *)calloc(1u, sizeof(*worker));
    if (worker == NULL) {
        snprintf(ui->status, sizeof(ui->status),
                 "IMPORT PREVIEW FAILED: OUT OF MEMORY STARTING DECODER");
        close_import_preview(device, audio, ui, controller);
        return 0;
    }
    ...
    worker->thread = SDL_CreateThread(import_worker_main,
                                      "TapeSister Import", worker);
    if (worker->thread == NULL) {
        snprintf(ui->status, sizeof(ui->status),
                 "IMPORT PREVIEW FAILED: COULD NOT START DECODER: %.86s",
                 SDL_GetError());
        ts_audio_import_free(&worker->decoded);
        free(worker);
        close_import_preview(device, audio, ui, controller);
    ...
    snprintf(ui->file_busy_label, sizeof(ui->file_busy_label), "DECODING");
    if (ui->browser.mode != TS_BROWSER_CLOSED)
        snprintf(ui->browser.message, sizeof(ui->browser.message),
                 "DECODING - ESC CANCELS");
    snprintf(ui->status, sizeof(ui->status),
             "DECODING %.115s - ESC OR CANCEL STOPS", path_basename(path));
    return 1;
}

static void cancel_import_decode(TsUiState *ui, ImportController *controller)
{
    if (ui == NULL || controller == NULL || controller->worker == NULL) return;
    discard_import_worker(controller);
    controller->path[0] = '\0';
    controller->selection_load = 0;
    controller->destination_slot = -1;
    ...
    ui->file_busy_label[0] = '\0';
    if (ui->browser.mode == TS_BROWSER_CLOSED)
        (void)ts_browser_open(&ui->browser, TS_BROWSER_LOAD_WAV, NULL);
    snprintf(ui->browser.message, sizeof(ui->browser.message),
             "IMPORT CANCELLED");
    snprintf(ui->status, sizeof(ui->status),
             "IMPORT DECODING CANCELLED - CHOOSE ANOTHER FILE");
}
...
static void poll_import_worker(SDL_AudioDeviceID device, AudioState *audio,
                               TsUiState *ui, TsInstrument *instrument,
                               TsSample *pending_selection_load,
                               ImportController *controller)
{
    ...
    char error[160];
    int bypass;
    if (ui == NULL || controller == NULL || controller->worker == NULL ||
        SDL_AtomicGet(&controller->worker->done) == 0) return;
    worker = controller->worker;
    SDL_WaitThread(worker->thread, NULL);
    controller->worker = NULL;
    ui->file_busy = 0;
    ...
    if (!worker->ok) {
        snprintf(error, sizeof(error), "%s",
                 worker->error[0] != '\0' ? worker->error :
                 "Audio decoder could not read this file");
        ts_audio_import_free(&worker->decoded);
        free(worker);
        controller->path[0] = '\0';
        controller->selection_load = 0;
        ...
        if (ui->browser.mode == TS_BROWSER_CLOSED)
            (void)ts_browser_open(&ui->browser, TS_BROWSER_LOAD_WAV, NULL);
        snprintf(ui->browser.message, sizeof(ui->browser.message),
                 "ERROR - SEE STATUS BAR");
        snprintf(ui->status, sizeof(ui->status),
                 "IMPORT PREVIEW FAILED: %.126s", error);
        return;
    }
    ts_audio_import_free(&controller->decoded);
    ...
    memcpy(ui->import_preview_maximum, worker->waveform_maximum,
           sizeof(ui->import_preview_maximum));
    ui->import_preview_waveform_ready = 1;
    ts_audio_import_free(&worker->decoded);
    free(worker);
    ...
    if (bypass) {
        apply_import_preview(device, audio, ui, instrument,
                             pending_selection_load, controller, 0);
        return;
    }
    snprintf(ui->status, sizeof(ui->status),
                                       ...
                                       ImportController *controller)
{
    if (controller != NULL && controller->worker != NULL) {
        cancel_import_decode(ui, controller);
        return;
    }
                                    ...
                                    int output_rate)
{
    ...
    audio->range_start = ui->import_preview_has_selection ?
                         ui->import_preview_selection_first : 0u;
    audio->range_end = ui->import_preview_has_selection ?
                       ui->import_preview_selection_last : sample->frames;
    if (audio->range_end <= audio->range_start ||
        audio->range_end > sample->frames) {
        audio->range_start = 0u;
        audio->range_end = sample->frames;
    }
    audio->position = ui->import_preview_playhead >= audio->range_start &&
                      ui->import_preview_playhead < audio->range_end ?
                      (double)ui->import_preview_playhead :
                      (double)audio->range_start;
    audio->pitch = 1.0;
    audio->source = TS_AUDITION_CURRENT;
    audio->range = TS_AUDITION_ALL;
    audio->looping = ui->import_preview_loop;
                                 ...
                                 int selection_only)
{
    ...
    TsAudioImport selected;
    const TsAudioImport *source = &controller->decoded;
    ts_audio_import_init(&selected);
    stop_import_preview(device, audio, ui, controller);
    if (selection_only) {
        if (!ui->import_preview_has_selection) {
            snprintf(ui->import_preview_message,
                     sizeof(ui->import_preview_message),
                     "DRAG ON THE WAVEFORM TO SELECT AN IMPORT RANGE");
            return;
        }
        if (!ts_audio_import_copy_range(
                &selected, &controller->decoded,
                ui->import_preview_selection_first,
                ui->import_preview_selection_last,
                error, sizeof(error))) {
            snprintf(ui->import_preview_message,
                     sizeof(ui->import_preview_message),
                     "SELECTION IMPORT FAILED: %.121s", error);
            ts_audio_import_free(&selected);
            return;
        }
        source = &selected;
    }
    if (controller->selection_load) {
        ...
        ok = ts_sample_clone(pending_selection_load, &source->sample,
                             error, sizeof(error));
        if (ok) {
    ...
    ok = ts_instrument_select_bank(instrument, destination, error, sizeof(error)) &&
         ts_instrument_import_sample(
             ...
             source->has_loop, source->loop_first,
             source->loop_last, source->loop_mode,
             error, sizeof(error));
    unlock_edit(device, audio, ui, instrument);
    if (!ok) {
        snprintf(ui->import_preview_message, sizeof(ui->import_preview_message),
    ...
    snprintf(ui->status, sizeof(ui->status),
             selection_only ?
             "IMPORTED %s SELECTION INTO BANK %02d %.67s" :
                                 ...
                                 int output_rate)
{
    ...
    if (action == TS_UI_IMPORT_ACTION_ACCEPT_SELECTION) {
        apply_import_preview(device, audio, ui, instrument,
                             pending_selection_load, controller, 1);
        return;
    }
                           ...
                           int bypass_preview)
{
    ...
    pending->bypass_preview =
        browser->mode == TS_BROWSER_LOAD_WAV && bypass_preview;
    ui->file_busy = 1;
...
static void external_input_request(ExternalInputState *input,
                                   TsInputConsumer consumer, int requested);
static int external_input_requested(const ExternalInputState *input,
                                    TsInputConsumer consumer);
static int sync_external_input_consumers(SDL_AudioDeviceID *input_device,
                                         AudioState *audio,
...
static void sync_sister_ext_consumer(SDL_AudioDeviceID *input_device,
                                     AudioState *audio,
                                     ExternalInputState *input,
                                     TsUiState *ui,
                                     SisterWindow *sister)
{
    ...
    int desired;
    int current;
    if (input_device == NULL || audio == NULL || input == NULL || ui == NULL)
        return;
    ...
    if (!sync_external_input_consumers(input_device, audio, input, &ui->config,
                                       error, sizeof(error)) && sister != NULL)
...
static int sister_window_ensure(SisterWindow *sister, const TsConfig *config)
{
    ...
    if (sister->renderer == NULL && sister->window != NULL)
        sister->renderer = SDL_CreateRenderer(sister->window, -1, SDL_RENDERER_SOFTWARE);
    ...
    if (sister->window == NULL || sister->renderer == NULL || sister->texture == NULL) {
        ...
        if (sister->window != NULL) SDL_DestroyWindow(sister->window);
        sister->window = NULL;
        sister->renderer = NULL;
        sister->texture = NULL;
        sister->window_id = 0u;
        return 0;
    }
                                 ...
                                 int parameter, float amount)
{
    ...
    case TS_SISTER_UI_PARAM_PRISM_TIME: parameters->prism.morph_seconds=ts_prism_time_from_normalized(amount);break;
    ...
    case TS_SISTER_UI_PARAM_H2_LEVEL: parameters->head2_level = amount; break;
    case TS_SISTER_UI_PARAM_H2_SCRUB: parameters->head2_scrub = amount; break;
    case TS_SISTER_UI_PARAM_H2_RATE: parameters->head2_rate_index = (int)lrintf(amount * 9.0f); break;
    case TS_SISTER_UI_PARAM_H2_FEEDBACK: parameters->head2_feedback = amount; break;
    case TS_SISTER_UI_PARAM_H3_LEVEL: parameters->head3_level = amount; break;
    case TS_SISTER_UI_PARAM_H3_SPAN: parameters->head3_span = amount; break;
    case TS_SISTER_UI_PARAM_H3_RATE: parameters->head3_rate_index = (int)lrintf(amount * 9.0f); break;
    case TS_SISTER_UI_PARAM_WOW: parameters->wow = amount * 10.0f; break;
    case TS_SISTER_UI_PARAM_DROP: parameters->drop = amount * 100.0f; break;
    case TS_SISTER_UI_PARAM_DUCK:
        parameters->duck_enabled = amount > 0.01f;
        parameters->duck_sensitivity = amount;
        break;
    case TS_SISTER_UI_PARAM_DECORRELATE:
        parameters->decorrelation_enabled = amount >= 0.5f;
        break;
    case TS_SISTER_UI_PARAM_WIDTH: parameters->width = amount; break;
    case TS_SISTER_UI_PARAM_FILTER_TYPE:
        parameters->filter_type = (TsSisterFilterType)lrintf(
            amount * (TS_SISTER_FILTER_TYPE_COUNT - 1));
        break;
    case TS_SISTER_UI_PARAM_FILTER_CUTOFF:
        parameters->filter_cutoff_hz = 20.0f * powf(1000.0f, amount);
        break;
    case TS_SISTER_UI_PARAM_FILTER_Q:
        parameters->filter_q = 0.1f + amount * 19.9f;
        break;
...
static float sister_parameter_normalized(const TsSisterParameters *parameters,
                                         int parameter)
{
    float value = 0.0f;
    int slot_offset;
    ...
    case TS_SISTER_UI_PARAM_PRISM_TIME: value=ts_prism_time_normalized(parameters->prism.morph_seconds);break;
    ...
    case TS_SISTER_UI_PARAM_H2_LEVEL: value = parameters->head2_level; break;
    case TS_SISTER_UI_PARAM_H2_SCRUB: value = parameters->head2_scrub; break;
    case TS_SISTER_UI_PARAM_H2_RATE: value = parameters->head2_rate_index / 9.0f; break;
    case TS_SISTER_UI_PARAM_H2_FEEDBACK: value = parameters->head2_feedback; break;
    case TS_SISTER_UI_PARAM_H3_LEVEL: value = parameters->head3_level; break;
    case TS_SISTER_UI_PARAM_H3_SPAN: value = parameters->head3_span; break;
    case TS_SISTER_UI_PARAM_H3_RATE: value = parameters->head3_rate_index / 9.0f; break;
    case TS_SISTER_UI_PARAM_WOW: value = parameters->wow / 10.0f; break;
    case TS_SISTER_UI_PARAM_DROP: value = parameters->drop / 100.0f; break;
    case TS_SISTER_UI_PARAM_DUCK: value = parameters->duck_sensitivity; break;
    case TS_SISTER_UI_PARAM_DECORRELATE: value = parameters->decorrelation_enabled ? 1.0f : 0.0f; break;
    case TS_SISTER_UI_PARAM_WIDTH: value = parameters->width; break;
    case TS_SISTER_UI_PARAM_FILTER_TYPE: value = parameters->filter_type / (float)(TS_SISTER_FILTER_TYPE_COUNT - 1); break;
...
static void fallout_factory_presets_configure(TsSisterPresetBank *bank,
                                              uint32_t sample_rate)
{
    static const char *const names[TS_SISTER_FACTORY_PRESET_COUNT] = {
        "APPROACHING TRAIN", "DUST WEATHER", "DEAD TRANSMISSION"
    };
    if (bank == NULL || bank->count < TS_SISTER_FACTORY_PRESET_COUNT) return;
    for (size_t index = 0u; index < TS_SISTER_FACTORY_PRESET_COUNT; ++index) {
        TsSisterFalloutControls *f;
        ts_sister_parameters_default(&bank->entries[index].parameters,
                                     sample_rate);
        f = &bank->entries[index].parameters.fx.fallout;
        ts_sister_fallout_controls_default(f);
        f->enabled = 0;
    ...
    {
        TsSisterFalloutControls *f =
            &bank->entries[0].parameters.fx.fallout;
        f->mix = 0.0f;
        f->feedback = 0.10f;
        f->noise = 0.75f;
        f->noise_type = TS_SISTER_FALLOUT_NOISE_BROWN;
        f->transition = ts_sister_fallout_transition_normalized(1000.0f);
        f->lfo_rate = ts_sister_fallout_lfo_normalized(8.0f);
        f->lfo_intensity = 0.95f;
        f->lfo_targets = TS_SISTER_FALLOUT_LFO_NOISE;
        f->rise_mode = TS_SISTER_FALLOUT_RISE_SAW;
        f->rise_length = ts_sister_fallout_rise_normalized(3600.0f);
        f->rise_intensity = 1.0f;
        f->rise_targets = TS_SISTER_FALLOUT_LFO_MIX;
    }
    {
        TsSisterFalloutControls *f =
            &bank->entries[1].parameters.fx.fallout;
        f->mix = 0.38f;
        f->feedback = 0.22f;
        f->noise = 0.28f;
        f->noise_type = TS_SISTER_FALLOUT_NOISE_PINK;
        f->transition = ts_sister_fallout_transition_normalized(5000.0f);
        f->pan_enabled = 1;
        f->pan_rate = 0.65f;
        f->lfo_rate = ts_sister_fallout_lfo_normalized(0.015f);
        f->lfo_intensity = 0.55f;
        f->lfo_targets = TS_SISTER_FALLOUT_LFO_NOISE |
                         TS_SISTER_FALLOUT_LFO_FEEDBACK;
        f->rise_mode = TS_SISTER_FALLOUT_RISE_ONE_SHOT;
        f->rise_length = ts_sister_fallout_rise_normalized(2700.0f);
        f->rise_intensity = 0.70f;
        f->rise_targets = TS_SISTER_FALLOUT_LFO_MIX |
                          TS_SISTER_FALLOUT_LFO_NOISE;
    }
    {
        TsSisterFalloutControls *f =
            &bank->entries[2].parameters.fx.fallout;
        f->mix = 0.72f;
        f->feedback = 0.30f;
        f->noise = 0.18f;
        f->noise_type = TS_SISTER_FALLOUT_NOISE_BLUE;
        f->transition = ts_sister_fallout_transition_normalized(2000.0f);
        f->skip_enabled = 1;
        f->skip_span = 0.18f;
        f->skip_rate = 0.35f;
        f->bit_enabled = 1;
        f->bit_quality = 0.32f;
        f->bit_resolution = 0.25f;
        f->bit_rate = 0.45f;
        f->lfo_rate = ts_sister_fallout_lfo_normalized(0.08f);
        f->lfo_intensity = 0.45f;
        f->lfo_targets = TS_SISTER_FALLOUT_LFO_NOISE |
                         TS_SISTER_FALLOUT_LFO_SKIP_SPAN |
                         TS_SISTER_FALLOUT_LFO_BIT_QUALITY;
        f->rise_mode = TS_SISTER_FALLOUT_RISE_ONE_SHOT;
        f->rise_length = ts_sister_fallout_rise_normalized(1800.0f);
        f->rise_intensity = 0.80f;
        f->rise_targets = TS_SISTER_FALLOUT_LFO_MIX |
                          TS_SISTER_FALLOUT_LFO_FEEDBACK;
    }
    for (size_t index = 0u; index < TS_SISTER_FACTORY_PRESET_COUNT; ++index)
        ts_sister_parameters_sanitize(&bank->entries[index].parameters,
                                      sample_rate);
}
    ...
    if (name != NULL && name[0] != '\0') {
        for (size_t i = 0u; i < sister->presets.count; ++i) {
            if (strcmp(sister->presets.entries[i].name, name) == 0) {
                sister->preset_index = i;
                break;
            }
        }
    }
    if (sister->preset_index < sister->presets.count) {
        const TsSisterPreset *preset =
            &sister->presets.entries[sister->preset_index];
        snprintf(sister->model.preset_name,
                 sizeof(sister->model.preset_name), "%s", preset->name);
        sister->model.preset_factory = preset->factory;
...
static void sister_recall_preset(SDL_AudioDeviceID device,
                                 AudioState *audio, SisterWindow *sister,
                                 size_t index)
{
...
static void sister_preset_edit_append(TsSisterUiModel *model,
                                ...
                                uint32_t sample_rate)
{
    ...
    if (sister->model.destination_mode == TS_SISTER_UI_DEST_NEXT_EMPTY)
        destination = ts_sister_runtime_find_destination(
            &audio->sister, instrument, destination);
    if (destination < 0) {
        snprintf(sister->model.status, sizeof(sister->model.status),
                 "NO EMPTY UNLOCKED TARGET ON THIS PAGE");
        return 0;
    }
    ...
    snprintf(sister->model.status, sizeof(sister->model.status), "%s",
             ok ? (overdub ? "OVERDUB RECORDING - PRESS AGAIN TO STOP" :
                            "CAPTURE RECORDING - PRESS AGAIN TO STOP") : error);
    return ok;
}
                                ...
                                uint32_t sample_rate, uint8_t output_channels)
{
    ...
    preset_index = fallout_preset_scope ?
        &sister->fallout_preset_index : &sister->preset_index;
    error[0] = '\0';
    ...
    if (hit.action == TS_SISTER_UI_ACTION_PRESET_PREVIOUS ||
        ...
        if (*preset_index >= preset_bank->count)
            index = hit.action == TS_SISTER_UI_ACTION_PRESET_NEXT ?
                    0u : preset_bank->count - 1u;
        else if (hit.action == TS_SISTER_UI_ACTION_PRESET_NEXT)
            index = (*preset_index + 1u) % preset_bank->count;
        else
            index = *preset_index == 0u ? preset_bank->count - 1u :
                    *preset_index - 1u;
        sister_recall_preset(device, audio, sister, index);
        return;
    }
    ...
    if (hit.action == TS_SISTER_UI_ACTION_PRESET_CANCEL) {
    ...
    if (hit.action == TS_SISTER_UI_ACTION_PRESET_SAVE_AS) {
    ...
    if (hit.action == TS_SISTER_UI_ACTION_PRESET_RENAME) {
        if (*preset_index >= preset_bank->count) {
            ...
            snprintf(sister->model.preset_edit_name,
                     ...
                     preset_bank->entries[*preset_index].name);
            sister->model.preset_edit_cursor =
                strlen(sister->model.preset_edit_name);
        }
        return;
    ...
    if (hit.action == TS_SISTER_UI_ACTION_PRESET_OVERWRITE ||
        ...
        if (*preset_index >= preset_bank->count) {
            ...
            sister->model.preset_editing = 0;
            sister->model.preset_confirmation =
                hit.action == TS_SISTER_UI_ACTION_PRESET_OVERWRITE ? 1 : 2;
        }
        return;
    }
    if (hit.action == TS_SISTER_UI_ACTION_PRESET_CONFIRM) {
        ...
        snprintf(sister->model.status, sizeof(sister->model.status), "%s",
                 ok ? (fallout_preset_scope ? "FALLOUT PRESET BANK SAVED" :
                                                "SISTER PRESET BANK SAVED") :
                      error);
        return;
    }
    ...
    case TS_SISTER_UI_ACTION_HOLD:
        ts_sister_runtime_set_hold(&audio->sister, !audio->sister.held); break;
    case TS_SISTER_UI_ACTION_CLEAR:
        if (!ts_sister_runtime_request_clear(&audio->sister))
            snprintf(sister->model.status, sizeof(sister->model.status),
                     "CLEAR ALREADY IN PROGRESS");
        break;
    case TS_SISTER_UI_ACTION_MONITOR:
        ts_sister_runtime_set_monitor(&audio->sister,
                                      !audio->sister.monitor_enabled); break;
    case TS_SISTER_UI_ACTION_SOURCE_TILES:
    ...
    if (sync_ext && input_device != NULL && external_input != NULL &&
        ...
        snprintf(sister->model.status, sizeof(sister->model.status),
                 "EXT - DEVICE UNAVAILABLE: %.91s", error);
    if (hit.action == TS_SISTER_UI_ACTION_WAVE_MODE) {
        sister->model.waveform_mode = ts_waveform_display_cycle(
                                   ...
                                   TsInputConsumer consumer, int requested)
{
    ...
    if (requested)
        (void)ts_input_ownership_request(&input->ownership, consumer);
    else
        (void)ts_input_ownership_release(&input->ownership, consumer);
}
...
{
    return input != NULL &&
           ts_input_ownership_requested(&input->ownership, consumer);
}
...
{
    if (input_device == NULL || input == NULL || config == NULL) return 0;
    if (input->ownership.requests == 0u) {
    ...
    if (*input_device == 0 &&
        !ensure_external_input_open(input_device, audio, input, config,
                                    error, error_size)) {
        ts_input_ownership_set_device(&input->ownership, 0, 0);
        return 0;
    }
                                     ...
                                     TsUiState *ui)
{
    ...
    if(input->recorder.stream && input->recorder.state==TS_EXTERNAL_CAPTURE_RECORDING &&
    ...
    if(input->recorder.stream) {
        ...
        size_t available=r->recorded_frames-input->waveform_consumed_frames;
        if(available>r->preview_capacity) {
            input->waveform_consumed_frames=r->recorded_frames-r->preview_capacity;
            available=r->preview_capacity;
        }
        if(available>8192)available=8192;
        preview_start=input->waveform_consumed_frames;
        for(size_t i=0;i<available;++i)for(int ch=0;ch<r->channels;++ch)
            stream_preview[i*r->channels+ch]=r->buffer[((preview_start+i)%r->preview_capacity)*r->channels+ch];
        ts_live_waveform_push_channels(&input->live_waveform,stream_preview,available,r->channels);
        input->waveform_consumed_frames+=available;
        preview_data=stream_preview;preview_frames=available;preview_channels=r->channels;
    } else if (input->recorder.recorded_frames > input->waveform_consumed_frames &&
    ...
    if(preview_start>ui->mosaic_record_preview.frames) {
        TsMosaicRecordPreview *p=&ui->mosaic_record_preview;
        while(p->frames_per_peak && preview_start/p->frames_per_peak>=TS_MOSAIC_RECORD_PEAKS) {
            for(int i=0;i<TS_MOSAIC_RECORD_PEAKS/2;++i)p->peaks[i]=fmaxf(p->peaks[i*2],p->peaks[i*2+1]);
            memset(p->peaks+TS_MOSAIC_RECORD_PEAKS/2,0,sizeof(p->peaks)/2);p->frames_per_peak*=2;
        }
        p->frames=preview_start;
    }
                                ...
                                int mosaic_target)
{
    ...
    if(streaming) {
        ...
        if(!ok) {
            ...
            snprintf(ui->status,sizeof(ui->status),"REC ARM FAILED: %.142s",error);return 0;
        }
    }
    ...
    if(streaming) {ts_external_recorder_free(&input->recorder);input->recorder=prepared;ok=1;}
    else ok = ts_external_recorder_arm_channels(
    ...
    if (ok && manual_source && !streaming) ok=ts_external_recorder_start_manual(&input->recorder);
                                    ...
                                    ExternalInputState *input, TsUiState *ui)
{
    ...
    if(input->recorder.stream) {
        ...
        if(source_device)SDL_UnlockAudioDevice(source_device);
        ts_performance_recorder_request_stop(input->recorder.stream);
        if(input->mosaic_writer) {SDL_WaitThread(input->mosaic_writer,NULL);input->mosaic_writer=NULL;}
        else while(ts_performance_recorder_pump(input->recorder.stream,8192)) {}
        remove(path);
        if(source_device)SDL_LockAudioDevice(source_device);
    }
                                    ...
                                    TsUiState *ui)
{
    ...
    if (ui->monitor_enabled) {
        ...
        atomic_store_explicit(&input->direct_monitor, 0,
                              memory_order_release);
        (void)ts_input_ownership_release(
            &input->ownership, TS_INPUT_CONSUMER_RECORD_MONITOR);
        (void)sync_external_input_consumers(
            input_device, audio, input, &ui->config, error, sizeof(error));
    ...
    atomic_store_explicit(&input->direct_monitor, 1,
                          memory_order_release);
    ...
    if (activate) {
        ...
        if (ui->monitor_enabled) {
            ...
            if (input_device) SDL_PauseAudioDevice(
                input_device, input->ownership.requests == 0u ? 1 : 0);
            if (input->ownership.requests == 0u)
                ts_input_monitor_set_enabled(&input->monitor, 0,
                                             input->sample_rate);
        }
        swap_instrument_storage(instrument, *parked);
...
int main(int argc, char **argv)
{
    ...
    ts_performance_recorder_init(&sister_window.performance_recorder);
    {
        ...
        if (sister_presets_file_path(preset_path, sizeof(preset_path)) &&
            !ts_sister_preset_load(&sister_window.presets, preset_path,
                                   48000u, preset_error,
                                   sizeof(preset_error)))
            fprintf(stderr, "TapeSister Sister presets: %s\n", preset_error);
        ts_sister_preset_bank_init(&sister_window.fallout_presets, 48000u);
        sister_window.fallout_preset_index = SIZE_MAX;
        if (fallout_presets_file_path(preset_path, sizeof(preset_path)) &&
            !ts_sister_preset_load(&sister_window.fallout_presets, preset_path,
                                   48000u, preset_error,
                                   sizeof(preset_error)))
            fprintf(stderr, "TapeSister Fallout presets: %s\n", preset_error);
        fallout_factory_presets_configure(
            &sister_window.fallout_presets, 48000u);
        snprintf(sister_window.model.preset_name,
        ...
        diagnostic_log("audio diagnostics disabled: atomics are not lock-free");
    {
        ...
        parameters.prism.morph_seconds = ui.config.prism_morph_seconds;
        parameters.prism.matrix.step_seconds = ui.config.prism_step_seconds;
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            if (event.type == SDL_AUDIODEVICEREMOVED &&
                ...
                if (atomic_load_explicit(&external_input.record_source,memory_order_acquire)==TS_RECORD_SOURCE_EXT &&
                    ...
                    if(external_input.recorder.stream && external_input.recorder.recorded_frames)
                        stop_external_capture_early(device,0,&external_input,&ui);
            ...
            if (event.type == SDL_DROPFILE && ui.warp_gesture.active) {
                ...
                } else if (import_controller.worker != NULL) {
                    if (key == SDLK_ESCAPE)
                        cancel_import_decode(&ui, &import_controller);
                    else
                        snprintf(ui.status, sizeof(ui.status),
                                 "DECODING IMPORT - ESC OR CANCEL STOPS");
                } else if (ui.fm_open) {
                ...
                if (ui.exit_confirm_open) {
                    ...
                    if (ui.browser.mode == TS_BROWSER_LOAD_WAV &&
                        ...
                        ui.browser.action_focus = -1;
                        if (event.button.clicks >= 2 ||
                            ((mod & KMOD_SHIFT) != 0 && index >= 0 &&
                             index < ui.browser.entry_count &&
                             !ui.browser.entries[index].is_directory))
                            browser_activate_selection(device, &audio, &ui, &instrument,
                                                       &pending_selection_load,
                                                       &sample_pages, parked_instrument,
                                                       record_bank_active, &pending_file,
                ...
                if (record_bank_active && bank_slot >= 0 &&
                    ...
                    snprintf(ui.status, sizeof(ui.status),
                       ...
                       (event.button.button == SDL_BUTTON_LEFT ||
                        event.button.button == SDL_BUTTON_RIGHT)) {
                if (ui.import_preview_selecting &&
                    event.button.button == SDL_BUTTON_LEFT) {
                    int x, y;
                    logical_mouse(window, event.button.x, event.button.y, &x, &y);
                    (void)y;
                    if (ui.import_preview_selection_dragged) {
                        ...
                        snprintf(ui.import_preview_message,
                                 ...
                                 "SELECTION READY - PLAY IT OR IMPORT SELECTION" :
        ...
        if (audio.sister.capture.state == TS_CAPTURE_COMPLETED) {
            ...
            if (committed) {
                ...
                snprintf(sister_window.model.status,
                         sizeof(sister_window.model.status),
                         "CAPTURE COMMITTED - ADD THE NEW TILE AS A SOURCE WHEN READY");
                snprintf(ui.status, sizeof(ui.status),
                         "SISTER %s CAPTURED TO TILE %02d",
                         ts_sister_tap_name(sister_window.model.selected_tap),
                         instrument.selected_slot + 1);
            } else {
                snprintf(sister_window.model.status,
                         sizeof(sister_window.model.status),
    ...
    if(external_input.recorder.stream) {
        ts_performance_recorder_request_stop(external_input.recorder.stream);
        if(external_input.mosaic_writer)SDL_WaitThread(external_input.mosaic_writer,NULL);
    }
    ...
    ts_sample_free(&drone_preview);
    ts_sample_free(&pending_selection_load);
    discard_import_worker(&import_controller);
    ts_audio_import_free(&import_controller.decoded);
    ts_sample_free(&clipboard);
    ts_sample_pages_free(&sample_pages);
    fm_bank_history_free(&fm_bank_history);
