                    ...
                    char *error, size_t error_size)
{
    if (slot >= 0 && ts_pr13_slot_locked(instrument, slot)) {
        set_error13(error, error_size,
                    "Family slot is locked - unlock it before editing");
        return 0;
    }
    return 1;
}
...
/*
 * A PR13 bank slot owns SOURCE audio plus an editable Recipe.  slot->sample is
 * deliberately never replaced by the rendered Current here.  That distinction
 * is what prevents Recipe settings from being baked into a tile every time the
 * user visits another family member.
 */
...
static int render_recipe(TsInstrument *instrument, int active_slot,
                         int permit_locked, int sync_slot,
                         char *error, size_t error_size)
{
    ...
    if (!permit_locked && !editable(instrument, active_slot, error, error_size)) return 0;
                          ...
                          char *error, size_t error_size)
{
    ...
    /* Core Set Current establishes slot->sample as the fresh Parent/source. */
    if (!ts_instrument_set_bank_as_current(instrument, active_slot,
                                           error, error_size)) return 0;
    ...
    /* Render the stored environment once, from the unprocessed slot source. */
    return render_recipe(instrument, active_slot, 1, 0, error, error_size);
}
                         ...
                         TsBankCaptureKind kind, char *error, size_t error_size)
{
    ...
    (void)active_slot;
    if (!ts_instrument_bank_capture(instrument, slot, kind,
                                    error, error_size)) return 0;
    /* A physical capture is already printed audio, so it starts with a neutral
       environment rather than re-applying the source tile's Recipe. */
    ts_pr13_neutral_process(&neutral);
    instrument->bank[slot].process = neutral;
                              ...
                              char *error, size_t error_size)
{
    ...
    if (!consolidate_if_full(instrument,
                             instrument->post_edit_count == 0,
                             instrument->post_edit_count > 0,
                             error, error_size)) return 0;
    if (!ts_instrument_apply_sample_edit(instrument, kind, amount,
                                         error, error_size)) return 0;
    return render_recipe(instrument, active_slot, 0, 1, error, error_size);
}
