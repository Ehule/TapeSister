...
static void pr13_drift(float *data, size_t frames, float drift)
{
    ...
    shift = lroundf((drift - 0.5f) * (float)frames);
    if (shift == 0) return;
...
int name args { \
    int ok; \
    if (!pr13_guard_edit(instrument, error, error_size)) return 0; \
    ok = legacy callargs; \
    if (ok) pr13_sync_active(instrument); \
    return ok; \
}
                             ...
                             char *error, size_t error_size)
{
    ...
    if (ts_instrument_bank_is_locked(instrument, slot)) {
        set_error(error, error_size, "Family slot is locked");
        return 0;
    }
...
PR13_BANK_GUARD(ts_instrument_bank_set_loop_crossfade, ts_instrument_bank_set_loop_crossfade_legacy,
    (TsInstrument *instrument, int slot, float milliseconds, char *error, size_t error_size), (instrument, slot, milliseconds, error, error_size))
PR13_BANK_GUARD(ts_instrument_bank_set_loop_mode, ts_instrument_bank_set_loop_mode_legacy,
    (TsInstrument *instrument, int slot, TsLoopMode mode, char *error, size_t error_size), (instrument, slot, mode, error, error_size))
...
{
    float minimum_similarity = pr13_similarity(relation);
    float inherited = 1.0f - clampf(mutation, 0.0f, 1.0f) * (1.0f - minimum_similarity);
    float target_peak;
    ...
    for (size_t i = 0; i < candidate->sample.frames; ++i) {
        ...
        float family = candidate->sample.data[i];
        float parent = anchor->sample.data[source_at];
        candidate->sample.data[i] = parent * inherited + family * (1.0f - inherited);
    }
                                            ...
                                            char *error, size_t error_size)
{
    ...
    ok = ts_instrument_generate_family_candidate_legacy(instrument, source_anchor, reseed, created_slot, error, error_size);
...
{
    FILE *file;
    unsigned char trailer[24] = {'P','R','1','3','L','K','1',0};
    int ok = ts_instrument_save_recipe_legacy(instrument, path, error, error_size);
    if (!ok || instrument == NULL) return ok;
    file = fopen(path, "ab");
    if (file == NULL) { set_error(error, error_size, "Saved project but could not append PR13 lock state"); return 0; }
    trailer[8] = (unsigned char)(instrument->active_bank_slot + 1);
    for (int i = 0; i < TS_BANK_SLOT_COUNT; ++i) trailer[9 + i] = (unsigned char)(instrument->bank[i].locked ? 1 : 0);
    if (fwrite(trailer, 1, sizeof(trailer), file) != sizeof(trailer)) { fclose(file); set_error(error, error_size, "Could not write PR13 lock state"); return 0; }
    fclose(file);
    set_error(error, error_size, "");
    return 1;
}
                              ...
                              char *error, size_t error_size)
{
    ...
    for (int i = 0; i < TS_BANK_SLOT_COUNT; ++i) instrument->bank[i].locked = 0;
    file = fopen(path, "rb");
    if (file != NULL && fseek(file, -(long)sizeof(trailer), SEEK_END) == 0 &&
        fread(trailer, 1, sizeof(trailer), file) == sizeof(trailer) &&
        memcmp(trailer, "PR13LK1", 7) == 0) {
        int active_slot = (int)trailer[8] - 1;
        for (int i = 0; i < TS_BANK_SLOT_COUNT; ++i) instrument->bank[i].locked = trailer[9 + i] ? 1 : 0;
        if (active_slot >= 0 && active_slot < TS_BANK_SLOT_COUNT && instrument->bank[active_slot].occupied)
            instrument->active_bank_slot = active_slot;
    }
