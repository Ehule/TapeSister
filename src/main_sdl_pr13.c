...
static void pr13_push_left_click(int x, int y, Uint32 which)
{
    SDL_Event queued;
    SDL_zero(queued);
    queued.type = SDL_MOUSEBUTTONDOWN;
    queued.button.type = SDL_MOUSEBUTTONDOWN;
    queued.button.which = which;
    queued.button.button = SDL_BUTTON_LEFT;
    queued.button.state = SDL_PRESSED;
    queued.button.clicks = 1;
    queued.button.x = x;
    queued.button.y = y;
...
static void pr13_remember_fine_control(int x, int y)
{
    ...
    if (y >= 233 && y < 257) {
        ...
        /* Page-specific continuous controls share this row. Keep nudges local to the
           clicked position; normal dragging remains the coarse gesture. */
        pr13_fine_min = x - 48;
        pr13_fine_max = x + 48;
...
{
    int got;
    if (pr13_restore_mods) {
        SDL_SetModState(pr13_saved_mods);
        pr13_restore_mods = 0;
    }
    got = SDL_PollEvent(event);
    if (!got || event == NULL) return got;

    if (event->type == SDL_MOUSEBUTTONDOWN && event->button.which == TS_PR13_SYNTHETIC)
        return got;
    ...
    if (event->type == SDL_MOUSEBUTTONDOWN && event->button.button == SDL_BUTTON_LEFT) {
        ...
        if (slot >= 0) {
            /* Clicking a Family member now means "this is the sound I am editing".
               The queued legacy Set Current action is internal plumbing; a second
               synthetic bank click restores the visible selected slot afterwards. */
            pr13_active_slot = slot;
            pr13_push_left_click(585, 216, TS_PR13_SYNTHETIC);
            pr13_push_left_click(x, y, TS_PR13_SYNTHETIC);
            return got;
        }
        if (y >= 205 && y < 228 && x >= 247 && x < 535) {
            /* Parent / Current / Commit / Reset are no longer state-selection controls. */
            event->type = SDL_USEREVENT;
            return got;
        }
        if (y >= 205 && y < 228 && x >= 540 && x < 630) {
            if (pr13_active_slot >= 0) {
                int column = pr13_active_slot % 8;
                int row = pr13_active_slot / 8;
                int bx = 10 + column * 77 + 36;
                int by = 330 + row * 25 + 12;
    ...
    if (event->type == SDL_MOUSEWHEEL && pr13_fine_active) {
        ...
        if (amount != 0) {
            ...
            event->button.which = TS_PR13_SYNTHETIC;
            event->button.button = SDL_BUTTON_LEFT;
            event->button.state = SDL_PRESSED;
            event->button.clicks = 1;
            event->button.x = pr13_fine_x;
            event->button.y = pr13_fine_y;
        }
        return got;
    }
