...
With no waveform selection, Create and Load replace only the selected tile. With a selection on an occupied sound or silent canvas, choosing a WAV from Load asks for exact Paste, duration-preserving Fit, or Cancel; Paste and Fit are both Undo/Redo edits. TSR and TSP loading retain their existing behavior. Clone makes an independent editable copy. Ordinary edits and WARP, SMEAR, and TEAR mutate only the selected tile. Create always proposes a fresh FM source. Vary changes that stored FM structure while it still describes the tile exactly; after Draw, Tune, Warp, Smear, Tear, Body, Edge, Drift, Paste, or other destructive shaping, Vary instead derives its answer from the current audible material and never adds an unrelated noise layer. With no selection, Chain-off Vary prints the response back into the selected tile as one Undo step; Chain-on Vary places it in the next empty tile and makes that result the next handoff. With a selection, Create stamps a fitted fresh FM sound, while Vary derives a fitted response from the sculpted selection. Chain-on Vary advances the same-width selection to the right with a tiny overlap and carries the preceding varied stamp forward as the next source.
...
prints the complete eight-second workspace render over the selected tile and resets that
tile's canvas to the same duration, regardless of its previous edits or canvas length.
With **Chain on**, it preserves occupied work and fills the next empty tile with that full
render. A full page asks whether to overwrite the current tile, continue on a new Sample
page, or cancel.
...
The default colors come from `assets/palette.pal`, using the complete shared TapeSister/Tapehead schema. The interface remains standalone: FT2 and the archived prototype are reference shelves, not inherited architecture, and TapeSister does not depend on or modify FT2 Tapehead Edition.
