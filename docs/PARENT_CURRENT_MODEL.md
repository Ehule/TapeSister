...
Root note and fine-tune cents are instrument metadata shared by Parent and Current, so A/B never compares two different keyboard mappings. The first onscreen C represents MIDI 48; its playback ratio is calculated against the instrument root rather than baked into either buffer. Tuning changes are ordinary undoable metadata edits, and active voices recalculate their step without restarting. Reset and Commit preserve this musical identity. A generated source initializes it from the generator's seeded base pitch; WAV import uses a standard `smpl` chunk when present and otherwise defaults to C3.
