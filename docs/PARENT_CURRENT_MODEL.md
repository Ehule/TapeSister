...
TSR8 is the native self-contained project container. It stores the embedded Parent audio, every occupied bank slot with integrity hashes and loop metadata, plus every field required to deterministically rebuild Current—including the ordered post-DSP tape timeline. Loading validates the complete container before replacing the live instrument. TSR6/TSR7 remain readable and default missing loop directions to Forward; TSR6 receives a root-only bank from its embedded Parent.

Before the first physical tape placement, Reverse, Normalize, gain, and fades are stored as deterministic selection-aware operations between Parent/Crop and the live DSP. A copy or move captures the rendered source into a temporary buffer, then replays its mix/overwrite placement after DSP; this makes overlap safe and keeps undo snapshots compact. Later sculpting and Crop join that ordered post-DSP timeline so edits continue to compose in the order the musician heard them. Noise, Delay, and Space still render only into Current. Reset is an undoable edit that clears crop, both edit timelines, and the DSP recipe so Current becomes sample-for-sample identical to Parent.
