...
| Mono/left/right/stereo/mix external modes | Controller/input device | Input monitor publishes sanitized stereo frames. Mono is exact dual mono; L/R selection and stereo ordering are explicit. Mode/device absence cannot invalidate Sister and EXT fades to silence. | Input ring is preallocated; callback reads bounded frames. | External-channel, input-monitor/ownership, stereo tests. MOTU ordering is manual. |
